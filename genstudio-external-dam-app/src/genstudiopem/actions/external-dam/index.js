...
const configureBucketAccess = (params) => {
  ...
  const region = params.AWS_REGION || process.env.AWS_REGION || "us-east-1";
  // console.log("===x params", params);
  // console.log("===x env", process.env);
...
async function getThumbnailUrl(s3Client, files, key, bucketName, logger) {
  try {
    ...
    if (files.exists(`public/${key}`)) {
      ...
      logger.info(
        "File does not exist in Adobe I/O Files, downloading from S3"
      );
    ...
    return errorResponse(
      500,
      "Error getting thumbnail URL: " + error.message,
      logger
    );
  }
}
...
async function searchAssets(params) {
  const logger = Core.Logger("searchAssets", {
  ...
  try {
    ...
    const assets = await Promise.all(
      listResult?.Contents.map(async (item) => {
        ...
        const originalUrl = await getS3PresignedUrl(
          client,
          item.Key,
          bucketName
        );
...
async function getAssetUrl(params) {
  const logger = Core.Logger("getAssetUrl", {
    level: params.LOG_LEVEL || "info",
  });
...
async function getAssetMetadata(params) {
  const logger = Core.Logger("getAssetMetadata", {
    level: params.LOG_LEVEL || "info",
  });
